...
class RidgeStacker:
    ...
    def fit(self, X, y):
        ...
        for n_fold, (tr_index, val_index) in enumerate(self.folds.split(X, y, X[self.group_col])):
            ...
            model = Ridge(alpha=self.alpha).fit(X_tr.drop(columns=self.cols_to_drop), y_tr)

            self.models.append(model)

            tr_pred = model.predict(X_tr.drop(columns=self.cols_to_drop))
            tr_pred = adjust_dist(tr_pred, tr_mean, tr_std)
            tr_pred = allocate_to_rate(tr_pred)
            ...
            self.scores['training'].append(qwk(tr_pred, y_tr))
            self.scores['valid_1'].append(qwk(val_pred, y_val))
